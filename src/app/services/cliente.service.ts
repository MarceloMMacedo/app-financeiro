...
export class ClienteService {
  ...
  findNamePage(name: string, page: number, linesPerPage: number, orderBy: string, direction: string): Observable<Pages> {
    let rota = API_CONFIG.baseUrl + '/clientes' + '/page?name=' + name + '&page=' + page +
  ...
  findperfil(email: string): Observable<BaseDto> {
    console.log(email);
    return this.http.get<BaseDto>(`${API_CONFIG.clientes}/email?value=${email}`);
  }

  findById(id: number): Observable<Pessoa> {

    return this.http.get<Pessoa>(`${API_CONFIG.clientes}/${id}`);
  }
  insert(p) {
    return this.http.post(`${API_CONFIG.clientes}`, p, { observe: 'response', responseType: 'text' });
  }

  getAll(): Observable<BaseDto[]> {
    return this.http.get<BaseDto[]>(`${API_CONFIG.clientes}/baseall`);
  ...
  save(p: Pessoa) {
    ...
    return this.http.put(`${API_CONFIG.clientes}/${p.id}`, p).subscribe(
      res => {
        setTimeout(() => {
  ...
  uploadPicture(p: Pessoa, event, prefixoAvatar: string): Pessoa {
    ...
    if (event.target.files && event.target.files) {
      ...
      const rota = API_CONFIG.clientes + '/' + p.id + '/picture';
  ...
  getrules(): Observable<string[]> {
    return this.http.get<string[]>(`${API_CONFIG.clientes}/perfils`);
  }
  getfuncoes(): Observable<string[]> {
    return this.http.get<string[]>(`${API_CONFIG.clientes}/funcoes`);
  }
}
